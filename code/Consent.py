...
model.add_adapter("CO", config=config)
model.set_active_adapters("CO")

model.add_classification_head("CO", num_labels=2)
...
tester = random.sample(list(set(data.File)), 5)
...
for epoch_i in list(range(0, epochs)):
    ...
    # train lyrik
    model.module.train_adapter("CO")
    train_batches = preprocess(train_data, bsize, "train", "CO")
        ...
        for file in list(set(test_data.File)):
            ...
            for text in tqdm(list(edata["Text"])):
                ...
                with torch.no_grad():
                    ...
                    l = softmax(outputs[0][0].detach().cpu().numpy())[1]
                    
                    prediction.append(l)
            print(max(prediction))
            prediction = [thres(x) for x in prediction]
            print(classification_report(edata["CO"], prediction))
            mylog+=classification_report(edata["CO"], prediction)

                
         
        
with open("logs/CO_"+run+".tsv", "w") as f:
    f.write(mylog)
          
model.module.save_adapter("models/CO/", "CO", with_head=True)