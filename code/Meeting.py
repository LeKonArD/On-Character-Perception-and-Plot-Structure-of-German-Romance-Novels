...
model.add_adapter("ME", config=config)
model.set_active_adapters("ME")

model.add_classification_head("ME", num_labels=2)
...
data = pd.read_csv("meeting_dataset.tsv", sep="\t", index_col=0)
tester = random.sample(list(set(data.File)), 3)
...
for epoch_i in list(range(0, epochs)):
    ...
    if (epoch_i+1) % 5 == 0:
        ...
        for file in list(set(test_data.File)):
            ...
            print(classification_report(edata["Label"], prediction))
            mylog+=classification_report(edata["Label"], prediction)

                
         
        
with open("logs/ME_"+run+".tsv", "w") as f:
    f.write(mylog)
          
model.module.save_adapter("models/ME/", "ME", with_head=True)